<?php
	...
	$Email = $_GET['email'];
		...
		$message    = "Tourist is looking for a tour and interested to contact through mail:<b><br />" . $Email . "</b><br /><br /> -----------------------------<br />";
		...
		if(SendMail($HostEmail, 'GuidedGateway', 'support@guidedgateway.com', 'Guided Gateway Support', $subject, $message))
		{
			...
			error_log($errormsg,0);
				if(SendMail($HostEmail, 'GuidedGateway', $Email, 'Tourist','thanx for contact', 'Thanx to contact with guided gateway team we will contact you soon'))
				{
					$errormsg="Acknowledgement email sent.";
					error_log($errormsg,0);
				}
