	...
	if(isset($_SESSION['userId']))
	{
		...
		else
		{
			...
			$_SESSION['phase'] = "signin";
				...
				$select4 = mysql_query("SELECT * FROM `tbl_tour_itinerary` WHERE `tour_id` = $tour_id");
				$count44 = mysql_num_rows($select4);
				$row44 = mysql_fetch_assoc($select4);
				if($count44 > 0 )
				{
				$tour_Itinerary_id = $row44["tour_Itinerary_id"];
				$tour_id = $row44["tour_id"];
				$day = $row44["day"];
				$intraday = $row44["intraday"];
				$description = $row44["description"];
				$transport = $row44["transport"];
				$tourist_spot = $row44["tourist_spot"];
				}
								...
								<div class="tab-content clearfix ">
									...
									<div class="tab-pane active" id="createTour">
										<div class="booking gray clearfix box-shadow1">
											...
											<form method="post" action="edit_Tour_Step2_code.php">
											<input type="hidden" name="userid" value="<?php echo $userid ?>" />
											...
											$tour_new_duration=$row33["tour_duration"];
											$data = array();
											for($f=1 ; $f <= $tour_new_duration ; $f++)
											{
												${'subSelect' . $f} = mysql_query("SELECT * FROM `tbl_tour_itinerary` WHERE `tour_id` = $tour_id and `day` = $f");
												${'dayCount' . $f} = mysql_num_rows(${'subSelect' . $f});
												$t=1;
												while(${'roww' . $f} = mysql_fetch_assoc(${'subSelect' . $f})) 
												{
													$data[$t]["intraday"] = ${'roww' . $f}["intraday"];
													$data[$t]["description"] = ${'roww' . $f}["description"];
													$data[$t]["transport"] = ${'roww' . $f}["transport"]; 
													$data[$t]["tourist_spot"] = ${'roww' . $f}["tourist_spot"];
													$t=$t+1;
												}
												//${'roww' . $f} = mysql_fetch_assoc(${'subSelect' . $f});
												
											?>
											<input type="hidden" name="tour_duration" value="<?php echo $tour_new_duration ?>" />
											<div class="row">
											<h1>Day <?php echo ($f); ?></h1>
											<div class="col-sm-12">
											<div class="form-group" style="overflow: auto">
											<table ID="tblPets<?php echo ($f); ?>" class="tbl-responsive" style="width:100%">
											<?php
											if( ${'dayCount' . $f} > 0 )
												{
											$p=1;
											for($g=1 ; $g <= ${'dayCount' . $f} ; $g++)
											{
											?>
											<tbody>
												<tr id="tblPets<?php echo ($f); ?>_TR<?php echo ($p); ?>">
												<td><h2>Visit : <?php echo ($g); ?></h2><br></td>
												<td align="right">
												<?php
												if($g == 1)
												{
												?>
												<input type="Button" class="btn btn-success btn-sm" id="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>" NAME="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>" onClick="addRow('tblPets<?php echo ($f); ?>')" value="Add New Visit">
												<?php
												}
												else
												{
												?>
												<INPUT class="btn btn-danger btn-sm" id= "tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>" NAME="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>"  TYPE="Button" CLASS="Button" onClick="delRow('tblPets<?php echo ($f); ?>',<?php echo ($p); ?>)" VALUE="Delete Visit">
												<?php
												}
												?>
												</td>
												...
												<tr id="tblPets<?php echo ($f); ?>_TR<?php echo ($p); ?>">
													...
													<td>
													<input class="form-control" style="background-color:white" required TYPE="text" value="<?php echo $data[$g]["intraday"]; ?>" id="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>" NAME="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>"  >
													</td>
												</tr>
												<?php $p=$p+1 ?>
												<tr id="tblPets<?php echo ($f); ?>_TR<?php echo ($p); ?>">
													...
													<td>
													<input class="form-control" style="background-color:white" required TYPE="text" value="<?php echo $data[$g]["description"]; ?>" id="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>" NAME="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>"  >
													</td>
												</tr>
												<?php $p=$p+1 ?>
												<tr id="tblPets<?php echo ($f); ?>_TR<?php echo ($p); ?>">
													...
													<td>
													<input class="form-control" style="background-color:white" required TYPE="text" value="<?php echo $data[$g]["transport"]; ?>" id="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>" NAME="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>"  >
													</td>
												</tr>
												<?php $p=$p+1 ?>
												<tr id="tblPets<?php echo ($f); ?>_TR<?php echo ($p); ?>">
													...
													<td>
													<input class="form-control" style="background-color:white" required TYPE="text" value="<?php echo $data[$g]["tourist_spot"]; ?>" id="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>" NAME="tblPets<?php echo ($f); ?>_input<?php echo ($p); ?>"  ><br>
													</td>
												</tr>
												<?php $p=$p+1 ?>
											</tbody>
											<?php
												...
												else
												{
													?>
													<tbody>
											<tr id="tblPets<?php echo $f; ?>_TR1">
											<td><h2>Visit : 1</h2><br></td>
											<td align="right"><input type="Button" class="btn btn-success btn-sm" id="tblPets<?php echo $f; ?>_input1" NAME="tblPets<?php echo $f; ?>_input1" onClick="addRow('tblPets<?php echo $f; ?>')" value="Add New Visit"></td>
											</tr>
											<tr id="tblPets<?php echo $f; ?>_TR2">
												<th style="width:15%">Hours Schedule</th>
												<td>
												<input class="form-control" style="background-color:white" required TYPE="text" id="tblPets<?php echo $f; ?>_input2" NAME="tblPets<?php echo $f; ?>_input2"  >
												</td>
											</tr>
											<tr id="tblPets<?php echo $f; ?>_TR3">
												<th style="width:15%">Description</th>
												<td>
												<input class="form-control" style="background-color:white" required TYPE="text" id="tblPets<?php echo $f; ?>_input3" NAME="tblPets<?php echo $f; ?>_input3"  >
												</td>
											</tr>
											<tr id="tblPets<?php echo $f; ?>_TR4">
												<th style="width:15%">Transport</th>
												<td>
												<input class="form-control" style="background-color:white" required TYPE="text" id="tblPets<?php echo $f; ?>_input4" NAME="tblPets<?php echo $f; ?>_input4"  >
												</td>
											</tr>
											<tr id="tblPets<?php echo $f; ?>_TR5">
												<th style="width:15%">Tourist Place</th>
												<td>
												<input class="form-control" style="background-color:white" required TYPE="text" id="tblPets<?php echo $f; ?>_input5" NAME="tblPets<?php echo $f; ?>_input5"  ><br>
												</td>
											</tr>
											<tbody>
											<?php
											...
											<div class="col-sm-12">
												<div class="col-sm-6 col-sm-offset-3">
													<input type="submit" tabindex="13" class="form-control btn btn-warning" value="Update Tour"  name="tourNmae" />
												</div>
											</div>
											</form>
										</div>
...
var visit = "2";
  ...
  //var count = document.getElementById(in_tbl_name).rows.length;
    var tbody = document.getElementById(in_tbl_name).getElementsByTagName("TBODY")[as];
    // create row
